...
document.addEventListener('DOMContentLoaded', function () {
  const formAddNewRecord = document.getElementById('mandateForm');
  ...
  document.querySelector('.create-new').addEventListener('click', function () {
    offCanvasTitle.textContent = window.translations.add_new_mandate;

    offCanvasEl = new bootstrap.Offcanvas(offCanvasElement);
    offCanvasEl.show();

    $('#logo_required_span').fadeIn();
  ...
  $(document).on('click', '.edit-btn', function () {
    ...
    $('#mandate_id').val(currentEditId);
    $('#name').val(name);
    $('#slogan').val(slogan);
    $('#year').val(year).trigger('change');

    $('#logo_required_span').fadeOut();

    offCanvasTitle.textContent = window.translations.edit_mandate;
  ...
  const fv = FormValidation.formValidation(formAddNewRecord, {
    fields: {
      name: {
        validators: {
          notEmpty: {
            message: window.translations.mandate_name_required
          }
        }
      },
      slogan: {
        validators: {
          notEmpty: { message: window.translations.mandate_slogan_required }
        }
      },
      year: {
        validators: {
          notEmpty: { message: window.translations.mandate_year_required }
        }
      }
    },
    plugins: {
  ...
  fv.on('core.form.valid', function () {
    ...
    const mandateId = $('#mandate_id').val();
    const updating = mandateId && mandateId != 0 ? true : false;
    const url = updating ? window.routes.update.replace(':id', mandateId) : window.routes.store;
    ...
    $.ajax({
      ...
      error: function (error) {
        ...
        $('#mandateForm .is-invalid').removeClass('is-invalid');
        $('#mandateForm .invalid-feedback').removeClass('d-block');

        if (respJson && respJson.errors) {
          const errors = respJson.errors;
          for (const key in errors) {
            const input = $('#mandateForm').find('[name="' + key + '"]');
  ...
  $(document).on('click', '.btn-delete', function () {
    ...
    Swal.fire({
      title: window.translations.mandate_delete_title,
      text: window.translations.mandate_delete_text,
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        $.ajax({
          ...
          success: function (response) {
            Swal.fire('Supprimé !', response.message || 'Le mandat a été supprimée.', 'success');
